...
func dumpWarningEvents(events []corev1.Event, limit int) []model.DumpEventSummary {
	...
	for _, event := range warnings {
		out = append(out, model.DumpEventSummary{Type: event.Type, Reason: event.Reason, Message: event.Message, ObjectKind: event.InvolvedObject.Kind, ObjectName: event.InvolvedObject.Name, Timestamp: eventTime(event)})
	}
	return out
}
