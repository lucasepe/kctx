...
func dumpSignals(entities []model.DumpEntity, events []model.DumpEventSummary, deployments []appsv1.Deployment, replicaSets []appsv1.ReplicaSet, statefulSets []appsv1.StatefulSet, daemonSets []appsv1.DaemonSet, jobs []batchv1.Job, services []corev1.Service, slices []discoveryv1.EndpointSlice, pvcs []corev1.PersistentVolumeClaim) []model.DumpSignal {
	...
	for _, entity := range entities {
		if entity.Kind == "Pod" {
			...
			case "CrashLoopBackOff":
				signals = append(signals, model.DumpSignal{Severity: "critical", Code: "pod_crashloop", Message: fmt.Sprintf("Pod %s/%s is restarting repeatedly", entity.Namespace, entity.Name), EntityID: entity.ID})
			case "ImagePullBackOff", "ErrImagePull":
				signals = append(signals, model.DumpSignal{Severity: "critical", Code: "image_pull_error", Message: fmt.Sprintf("Pod %s/%s has image pull error %s", entity.Namespace, entity.Name, entity.Status), EntityID: entity.ID})
			}
			if entity.Ready != nil && !*entity.Ready {
				signals = append(signals, model.DumpSignal{Severity: "warning", Code: "pod_not_ready", Message: fmt.Sprintf("Pod %s/%s is not Ready", entity.Namespace, entity.Name), EntityID: entity.ID})
			}
		}
	}
	...
	for _, pvc := range pvcs {
		if pvc.Status.Phase == corev1.ClaimPending || pvc.Status.Phase == corev1.ClaimLost {
			signals = append(signals, model.DumpSignal{Severity: "warning", Code: "pvc_pending", Message: fmt.Sprintf("PVC %s/%s is %s", pvc.Namespace, pvc.Name, pvc.Status.Phase), EntityID: dumpID("PersistentVolumeClaim", pvc.Namespace, pvc.Name)})
		}
	}
...
func dumpEventSignals(events []model.DumpEventSummary) []model.DumpSignal {
	...
	for _, group := range groups {
		...
		signals = append(signals, model.DumpSignal{
			...
			Code:     dumpEventSignalCode(event),
...
func dumpWorkloadSignals(deployments []appsv1.Deployment, replicaSets []appsv1.ReplicaSet, statefulSets []appsv1.StatefulSet, daemonSets []appsv1.DaemonSet, jobs []batchv1.Job) []model.DumpSignal {
	...
	for _, deploy := range deployments {
		...
		if deploy.Status.AvailableReplicas < desired {
			...
			signals = append(signals, model.DumpSignal{Severity: "warning", Code: "workload_unavailable", Message: fmt.Sprintf("Deployment %s/%s available %d/%d", deploy.Namespace, deploy.Name, deploy.Status.AvailableReplicas, desired), EntityID: id})
		}
	}
	for _, rs := range replicaSets {
		...
		if rs.Status.ReadyReplicas < desired {
			...
			signals = append(signals, model.DumpSignal{Severity: "warning", Code: "workload_unavailable", Message: fmt.Sprintf("ReplicaSet %s/%s ready %d/%d", rs.Namespace, rs.Name, rs.Status.ReadyReplicas, desired), EntityID: id})
		}
	}
	for _, sts := range statefulSets {
		...
		if sts.Status.ReadyReplicas < desired {
			...
			signals = append(signals, model.DumpSignal{Severity: "warning", Code: "workload_unavailable", Message: fmt.Sprintf("StatefulSet %s/%s ready %d/%d", sts.Namespace, sts.Name, sts.Status.ReadyReplicas, desired), EntityID: id})
		}
	}
	for _, ds := range daemonSets {
		if ds.Status.NumberAvailable < ds.Status.DesiredNumberScheduled {
			id := dumpID("DaemonSet", ds.Namespace, ds.Name)
			signals = append(signals, model.DumpSignal{Severity: "warning", Code: "workload_unavailable", Message: fmt.Sprintf("DaemonSet %s/%s available %d/%d", ds.Namespace, ds.Name, ds.Status.NumberAvailable, ds.Status.DesiredNumberScheduled), EntityID: id})
		}
	}
	for _, job := range jobs {
		...
		if job.Status.Failed > 0 || job.Status.Succeeded < desired {
			...
			signals = append(signals, model.DumpSignal{Severity: "warning", Code: "workload_unavailable", Message: fmt.Sprintf("Job %s/%s succeeded %d/%d failed %d", job.Namespace, job.Name, job.Status.Succeeded, desired, job.Status.Failed), EntityID: id})
		}
	}
	return signals
}
...
func dumpServiceSignals(services []corev1.Service, slices []discoveryv1.EndpointSlice) []model.DumpSignal {
	...
	for _, service := range services {
		...
		if readyByService[service.Name] == 0 {
			...
			signals = append(signals, model.DumpSignal{Severity: "warning", Code: "service_without_ready_endpoints", Message: fmt.Sprintf("Service %s/%s has no ready endpoints", service.Namespace, service.Name), EntityID: id})
		}
	}
	return signals
}
