...
func podStatus(pod *corev1.Pod) model.PodStatus {
	...
	for _, condition := range pod.Status.Conditions {
		status.Conditions = append(status.Conditions, model.PodCondition{
			...
			Message: redaction.Text(condition.Message),
		})
	}
...
func containerState(state corev1.ContainerState) (string, string, string) {
	...
	case state.Waiting != nil:
		return "waiting", state.Waiting.Reason, redaction.Text(state.Waiting.Message)
	case state.Running != nil:
		return "running", "", ""
	case state.Terminated != nil:
		return "terminated", state.Terminated.Reason, redaction.Text(state.Terminated.Message)
	default:
		return "", "", ""
	}
}
