...
func namespaceSignals(namespace string, pods []model.PodHealth, workloads []model.WorkloadHealth, services []model.ServiceHealth, pvcs []model.PVCHealth, events []model.EventSummary, rawPodCount, workloadCount, serviceCount, pvcCount int) []model.Signal {
	...
	for _, pod := range pods {
		...
		case "CrashLoopBackOff":
			signals = append(signals, model.Signal{Severity: "error", Reason: "pod_crashloop", Message: fmt.Sprintf("Pod %s/%s is in CrashLoopBackOff", pod.Namespace, pod.Name), Source: source})
		case "ImagePullBackOff", "ErrImagePull":
			signals = append(signals, model.Signal{Severity: "error", Reason: "pod_image_pull_error", Message: fmt.Sprintf("Pod %s/%s has image pull error %s", pod.Namespace, pod.Name, pod.Reason), Source: source})
		}
