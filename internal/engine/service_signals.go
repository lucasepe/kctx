...
func traceSignals(service *corev1.Service, pods []model.PodBackend, endpoints []model.EndpointSummary) []model.Signal {
	...
	if len(pods) > 0 && readyPods == 0 {
		signals = append(signals, model.Signal{Severity: "error", Reason: "all_backends_unready", Message: fmt.Sprintf("all selected Pods for service %s/%s are unready", service.Namespace, service.Name), Source: "pod"})
	}
	if service.Spec.Type != corev1.ServiceTypeExternalName && readyEndpoints == 0 {
		signals = append(signals, model.Signal{Severity: "error", Reason: "service_has_no_usable_backends", Message: fmt.Sprintf("service %s/%s has no ready endpoints", service.Namespace, service.Name), Source: "endpoints"})
	}
