...
func buildSignals(status model.PodStatus, events []model.Event) []model.Signal {
	...
	for _, container := range status.Containers {
		...
		if container.Reason == "CrashLoopBackOff" {
			signals = append(signals, model.Signal{Severity: "critical", Reason: "container_crashloop", Message: "container " + container.Name + " is restarting repeatedly", Source: "status"})
		}
		if container.Reason == "ImagePullBackOff" || container.Reason == "ErrImagePull" {
			signals = append(signals, model.Signal{Severity: "critical", Reason: "image_pull_error", Message: "container " + container.Name + " cannot pull its image: " + container.Reason, Source: "status"})
		}
		if container.LastStateReason == "OOMKilled" {
			signals = append(signals, model.Signal{Severity: "critical", Reason: "container_oom_killed", Message: "container " + container.Name + " was previously OOMKilled", Source: "status"})
