...
func (e *TypedEngine) NamespaceHealth(ctx context.Context, req NamespaceHealthRequest) (*NamespaceHealthResponse, error) {
	...
	resp := &NamespaceHealthResponse{SchemaVersion: model.SchemaVersion, Kind: "NamespaceHealth", Namespace: req.Namespace}
	...
	normalizeNamespaceHealthSlices(resp)
	resp.Summary = namespaceSummary(resp)
	sortNamespaceHealth(resp)
	return resp, nil
}

func normalizeNamespaceHealthSlices(resp *NamespaceHealthResponse) {
	if resp.Workloads == nil {
		resp.Workloads = []model.WorkloadHealth{}
	}
	if resp.Pods == nil {
		resp.Pods = []model.PodHealth{}
	}
	if resp.Services == nil {
		resp.Services = []model.ServiceHealth{}
	}
	if resp.PVCs == nil {
		resp.PVCs = []model.PVCHealth{}
	}
	if resp.Events == nil {
		resp.Events = []model.EventSummary{}
	}
	if resp.Signals == nil {
