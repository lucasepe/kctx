...
func (e *TypedEngine) DumpNamespace(ctx context.Context, req DumpNamespaceRequest) (*DumpNamespaceResponse, error) {
	...
	normalizeNamespaceDumpSlices(resp)
...
func normalizeNamespaceDumpSlices(resp *DumpNamespaceResponse) {
	if resp.Entities == nil {
		resp.Entities = []model.DumpEntity{}
	}
	if resp.Relations == nil {
		resp.Relations = []model.DumpRelation{}
	}
	if resp.Signals == nil {
		resp.Signals = []model.DumpSignal{}
	}
	if resp.Events == nil {
		resp.Events = []model.DumpEventSummary{}
	}
}
