...
func TestHealthz(t *testing.T) {
	...
	assertBodyContains(t, rec, `"status": "ok"`)
}
...
func TestGraphPodMermaid(t *testing.T) {
	rec := get(t, testHandler(t, fixtureObjects()...), "/graph/pod/payments/api-1?format=mermaid")

	assertStatus(t, rec, http.StatusOK)
	assertContentType(t, rec, "text/plain")
	assertBodyContains(t, rec, "graph TD")
	assertBodyContains(t, rec, "-->|owns|")
}

func TestGraphPodDOT(t *testing.T) {
	rec := get(t, testHandler(t, fixtureObjects()...), "/graph/pod/payments/api-1?format=dot")

	assertStatus(t, rec, http.StatusOK)
	assertContentType(t, rec, "text/plain")
	assertBodyContains(t, rec, "digraph G")
	assertBodyContains(t, rec, `[label="owns"]`)
}
...
func TestJSONErrorResponses(t *testing.T) {
	...
	if body.Error != "not_found" {
		t.Fatalf("unexpected error code %q in %s", body.Error, rec.Body.String())
	}
}
...
func TestMethodNotAllowed(t *testing.T) {
	...
	assertContentType(t, rec, "application/json")
	assertBodyContains(t, rec, `"error":"method_not_allowed"`)
}

func testHandler(t *testing.T, objects ...runtime.Object) http.Handler {
	t.Helper()
	reader := testutil.NewFakeReader(objects...)
	return server.New(engine.New(reader), slog.Default()).Handler()
}
