...
func (c *explainCmd) Usage() string {
	...
	fmt.Fprint(&w, "USAGE:\n\n")
	fmt.Fprintf(&w, "  %s %s pod <name> [--namespace namespace] [--output human|json]\n\n", c.appName, c.Name())
	...
	fmt.Fprintf(&w, "  %s %s po api-xyz --namespace payments --output json\n\n", c.appName, c.Name())
...
func (c *explainCmd) render(env *cmd.Env, result *engine.ResolveContextResponse) error {
	...
	case "human":
		if result.Pod != nil {
			return render.HumanPodContext(env.Out, result.Pod)
		}
