...
func (c *Command) Synopsis() string {
	return "Dump a normalized namespace snapshot as JSON"
}
...
func (c *Command) Usage() string {
	...
	fmt.Fprintf(&w, "%s\n\n", c.Synopsis())
	fmt.Fprint(&w, "USAGE:\n\n")
	fmt.Fprintf(&w, "  %s dump namespace <namespace> --output json\n\n", c.appName)
	fmt.Fprint(&w, "DESCRIPTION:\n\n")
	fmt.Fprintln(&w, "  Exports a compact operational snapshot: normalized entities, relations,")
	fmt.Fprintln(&w, "  signals, and recent warning Events. Secret and ConfigMap data are not dumped.")
	fmt.Fprintln(&w)
	fmt.Fprint(&w, "EXAMPLES:\n\n")
	fmt.Fprintf(&w, "  %s dump namespace payments --output json\n\n", c.appName)
	return w.String()
}
...
func (c *Command) Execute(ctx context.Context, fs *flag.FlagSet, args ...any) cl.ExitStatus {
	...
	result, err := eng.DumpNamespace(ctx, engine.DumpNamespaceRequest{Namespace: c.namespace})
	if err != nil {
		return env.Fail(c.Name(), err, log.String("namespace", c.namespace))
	}
	if err := render.JSON(env.Out, result); err != nil {
		return env.Fail(c.Name(), err, log.String("namespace", c.namespace))
	}
