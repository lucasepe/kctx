...
import (
	...
	"github.com/lucasepe/kctx/internal/engine/adapters"
...
func NewEngine(kubeconfig string, inCluster bool) (*engine.Engine, error) {
	...
	return engine.NewWithDynamic(reader, reader, adapters.Default()...), nil
}
...
func (env *Env) FailJSON(task string, err error, fields ...log.Field) cl.ExitStatus {
	_ = render.JSON(env.Out, apperror.Envelope(err))
	return env.Fail(task, err, fields...)
}
